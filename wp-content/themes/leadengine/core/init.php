...
// ------------------------------------------------------------------------
	...
	// include_once( get_template_directory() . '/core/redux-extensions.php' );
...
function keydesign_transparent_navigation($classes) {
    $page_transparent_navigation = get_post_meta( get_the_ID(), '_themetek_page_transparent_navbar', true );
    if ( !empty($page_transparent_navigation)) {
	    $classes[] = 'transparent-navigation';
    }
    return $classes;
}
add_filter('body_class', 'keydesign_transparent_navigation');
